#Week 4. Clustering and classification. Álvaro Germán Torres Mora
#Analysis exercises
...
#STEP 4. Standardizing the dataset.
...
#STEP 5. Fitting the linear discriminant analysis on the train set.
#Code:
library(MASS)
...
#STEP 6. Predicting the classes with the LDA model on the test data.
#Code:
...
#Since we have defined the categories of crimes we have the variables the highest aiming for both the correct and the predicted in high. This means that the prediction based on the model works quite well.

#STEP 7. k-means algorithm on the dataset
#Code:
data(Boston)
boston_scaled <- scale(Boston)
...
#Data from the same cluster have continuity and they approximate each other in the distributions.
